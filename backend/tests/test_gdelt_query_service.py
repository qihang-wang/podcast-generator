...
测试 GDELTQueryService 的地理位置查询方�?

⚠️ 成本安全说明�?
本测试文件使用完�?Mock 的方式测�?GDELT 查询功能，不会产生任�?BigQuery 成本�?
- 所有测试使�?@patch 装饰�?Mock BigQuery 客户�?
- 不会建立真实的网络连�?
- 不会扫描任何实际数据
- total_bytes_processed 统计为手动设置的模拟�?
...
from gdelt.gdelt_service import GDELTQueryService
from gdelt.gdelt_event import EventQueryBuilder
from gdelt.model import EventModel


class TestGDELTQueryService(unittest.TestCase):
    """测试 GDELTQueryService �?""
    ...
    def test_query_events_by_location_geo_filtering(self, mock_bigquery):
        ...
        service = GDELTQueryService(config=self.mock_config)
        # 使用默认参数，应该包�?geo_types=[3, 4] �?require_feature_id=True
